...
def read_hrnet_wHand(joint_data, gt_part=None, dataset=None):
    ...
    if len(joint_data) == 0:
        ...
        op_output["pose_keypoints_2d"][:, -1] = np.where(op_output["pose_keypoints_2d"][:, -1] > .2,
        ...
        op_output["hand_right_keypoints_2d"][:, -1] = np.where(op_output["hand_right_keypoints_2d"][:, -1] > .2,
                op_output["hand_right_keypoints_2d"][:, -1],
                0)
        op_output["hand_left_keypoints_2d"][:, -1] = np.where(op_output["hand_left_keypoints_2d"][:, -1] > .2,
